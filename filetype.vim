...
" Batch file for MSDOS or .cmd file for Sentaurus.
au BufNewFile,BufRead *.bat,*.sys		setf dosbatch
" *.cmd is close to a Batch file, but on OS/2 Rexx files also use *.cmd.
" If one of first five lines is a scheme or tcl comment, uses that format
au BufNewFile,BufRead *.cmd
	\ if getline(1) =~ '^/\*' | setf rexx |
    \ elseif getline(1).getline(2).getline(3).getline(4).getline(5) =~ '^;' | setf scheme |
    \ elseif getline(1).getline(2).getline(3).getline(4).getline(5) =~ '^[#\*]' | setf tcl |
    \ else | setf dosbatch | endif
